...
from dataset_less_memory import *
import logging
import os
...
def main():
    ...
    train_step = tf.train.AdamOptimizer(0.0000005).minimize(loss, global_step=global_step) #小数点后7个0
    ...
    with tf.Session() as sess:
        ...
        left_train,left_dev,right_train,right_dev,similar_train,similar_dev = get_dataset(pairs_file,devset_need_or_not = True)
        left_dev_arr, right_dev_arr, similar_dev_arr = get_batch_image_array(left_dev, right_dev, similar_dev)
        
        print('num of left train is '+str(len(left_train)))
        print('num of right train is '+str(len(left_train)))
        print('num of similar train is '+str(len(left_train)))
        # train iter
        idx = 0
        ...
        for i in range(FLAGS.train_iter):
            ...
            if file_next_or_not == 1:
                pairs_file = os.path.join(pairs_base_path,str(pairs_files[pairs_txt_now]).strip())
                print('\n\n------next batch is from ' + str(pairs_files[pairs_txt_now]) +  ' .------\n\n')
                pairs_txt_now += 1
                if pairs_txt_now == len(pairs_files):
                    pairs_txt_now = 0

                left_train,right_train,similar_train = get_dataset(pairs_file,devset_need_or_not = False)
